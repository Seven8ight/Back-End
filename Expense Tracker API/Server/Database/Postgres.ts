...
interface User {
  ...
  loginUser: (
    userDetails: Omit<userDetails, "name">
...
interface Expense {
  ...
  updateExpense: (
    expenseId: string,
    expenseDetails: Partial<expense>
  ) => Promise<string>;
...
export class UserDB extends PostGresDb implements User {
  ...
  async createUser(userDetails: userDetails): Promise<string | tokens> {
    const userKeys = ["name", "email", "password"],
      checkForKeys = userKeys.every((key) =>
        Object.keys(userDetails).includes(key)
      );

    if (!checkForKeys) return "Ensure all keys are present in the json body";
    else {
      for (let [key, value] of Object.entries(userDetails)) {
        if (key == "email") {
          if (value.length < 0) return "Email length invalid";
          else if (
            /^[\w\.-]+@[a-zA-Z\d\.-]+\.[a-zA-Z]{2,}$/.test(value) == false
          )
          ...
          if (value.length <= 0) return "Name is invalid";
        }
      }
      ...
      try {
        ...
        if (userToken instanceof Error == false) {
          if (tableExists)
            ...
            await this.client.query(
              "CREATE TABLE users(id UUID,name TEXT,email TEXT UNIQUE,password TEXT)"
            );
            await this.client.query(
              "INSERT INTO users(id,name,email,password) VALUES($1,$2,$3,$4)",
              [userId, ...Object.values(userDetails)]
  ...
  async updateUser(userDetails: Partial<userDetails>): Promise<string> {
    ...
    if (!tableExists) return "User table doesn't exist";
      ...
      if (user instanceof Object) {
        try {
          ...
          if (expenseDeletion != "Expenses deleted")
            ...
            await this.client.query("DELETE FROM users WHERE id::text=$1", [
              (user as any).id,
            ]);
  ...
  async getUser(): Promise<userDetails | string> {
    ...
    if (user instanceof Object)
      try {
        if (tableExists) {
          let userObject = await this.client.query(
  ...
  async loginUser(
    ...
    if (!keyChecker) return "Provide email and password in request body";

    try {
      const userRetrieval: QueryResult<any> = await this.client.query(
      ...
      if (userRetrieval.rowCount && userRetrieval.rowCount > 0) {
        ...
        if (user.password == userDetails.password) {
          ...
          if (userToken instanceof Error) return userToken.message;
...
export class ExpenseDB extends UserDB implements Expense {
  ...
  async createExpense(expense: expense): Promise<string> {
    ...
    if (userObject instanceof Object) {
      const tableExists = await this.tableExists("expenses"),
        ...
        checkKeysPresent = expenseKeys.every((key) =>
          Object.keys(expense).includes(key)
        );

      if (!tableExists) {
        try {
          await this.client.query(
            "CREATE TABLE expenses(id UUID NOT NULL PRIMARY KEY, user_id UUID NOT NULL REFERENCES users(id),title TEXT NOT NULL,description TEXT, category TEXT,amount INT, created_at DATE,updated_at DATE)"
          );
        } catch (error) {
      ...
      if (checkKeysPresent) {
        for (let [key, value] of Object.entries(expense)) {
          if (key == "amount") {
            if ((value as number) < 0) return "Amount cannot be negative";
        ...
        try {
          await this.client.query(
            "INSERT INTO expenses(id,user_id,title,description,category,created_at,updated_at,amount) VALUES($1,$2,$3,$4,$5,$6,$7,$8);",
            [
              expenseId,
        ...
        return `Incomplete details, ensure to provide title,description, category, amount, createdAt and updatedAt fields`;
  ...
  async getExpense(expenseId: string): Promise<expense | string> {
    ...
    if (userObject instanceof Object) {
      try {
        const expenseQuery: QueryResult<expense> = await this.client.query(
          "SELECT * FROM expenses WHERE id::text=$1 AND user_id::text=$2",
  ...
  async getExpenses(): Promise<expense[] | string> {
    ...
    if (userObject instanceof Object) {
      try {
        const expensesQuery: QueryResult<expense> = await this.client.query(
          "SELECT e.id,e.title,e.description,e.category,e.amount,e.created_at,e.updated_at FROM expenses e INNER JOIN USERS u ON e.user_id=u.id"
        );
    ...
    if (userObject instanceof Object) {
      try {
        for (let [key, value] of Object.entries(expenseDetails)) {
          ...
          await this.client.query(
            `UPDATE expenses SET ${key}=$1 WHERE id::text=$2 AND user_id::text=$3`,
            [value, expenseId, (userObject as any).id]
          );
          await this.client.query(
            `UPDATE expenses SET updated_at=$1 WHERE id::text=$2 AND user_id::text=$3`,
  ...
  async deleteExpense(expenseId: string): Promise<string> {
    ...
    if (userObject instanceof Object) {
      try {
        await this.client.query(
          "DELETE FROM expenses WHERE id::text=$1 AND user_id::text=$2",
          [expenseId, (userObject as any).id]
        );
  ...
  async deleteExpenses(): Promise<string> {
    ...
    if (userObject instanceof Object) {
      try {
        await this.client.query("DELETE FROM expenses WHERE user_id::text=$1", [
          (userObject as any).id,
        ]);
